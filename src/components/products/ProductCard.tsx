...
export const ProductCard = ({ product, onClick } : ProductCardProps) => {
    return (
        ...
        <div className="text-center mb-1">
            <h3 className="font-medium leading-snug text-center text-lg mb-1 cursor-pointer">{product.name}</h3>
            <p className=" text-blue-800 font-semibold text-lg mb-1">{product.price.toLocaleString("es-CL", {style: "currency", currency: "CLP", minimumFractionDigits: 0, maximumFractionDigits: 2,})} </p>
        </div>

        {/* Controles */}
        <div className="flex items-center justify-center gap-2 mt-1">
            <button className="cursor-pointer bg-gray-600 text-white px-2 py-0.5 rounded-full hover:bg-gray-800 transition">-</button>
            <span className="bg-white px-3 py-1 rounded border select-none">1</span>
            <button className="cursor-pointer bg-gray-600 text-white px-2 py-1 rounded-full hover:bg-gray-800 transition">+</button>
        </div>
