...
export default function ViewProductsPage() {
    const { products, loading, fetchProducts, filters, error } = useProductStore(); 
    ...
    return (
      ...
      <div className="w-full bg-white border-b px-6 sm:px-6 py-3 sm:py-4 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <h2 className="text-base sm:text-xl font-bold text-gray-800 tracking-tight">Catálogo</h2>
          <ProductFiltersDrawer />
        </div>
      </div>

        <div className='max-w-7xl mx-auto px-4 py-12 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8'> {products.map((product) => (
        ...
        <div className="flex justify-center gap-4 mt-12 mb-8">
          <button onClick={() => console.log("Anterior")} className="bg-gray-200 hover:bg-gray-300 text-black px-4 py-2 rounded shadow transition duration-200 cursor-pointer" >
            Anterior
          </button>
          <button onClick={() => console.log("Siguiente")} className="bg-gray-200 hover:bg-gray-300 text-black px-4 py-2 rounded shadow transition duration-200 cursor-pointer" >
            Siguiente
          </button>
        </div>
